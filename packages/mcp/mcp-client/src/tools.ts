 ...
 *    (network error, repeated continuation cursor, duplicate raw name in the
 *    server's list) rejects and leaves the previous generation registered
 *    untouched.
...
export async function syncTools(
  ...
  do {
    ...
    if (nextCursor && seenCursors.has(nextCursor)) {
      throw new Error(
        `mcp-client(${opts.serverName}): server repeated a tools/list continuation cursor`,
      )
    }
