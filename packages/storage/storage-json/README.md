...
- A failure before atomic replacement rolls the mutation out of memory and leaves the unit usable. If parent-directory synchronization fails after `rename()` makes the replacement visible, the unit rejects with `closed` and releases its open slot; reopening reads the medium instead of letting cached pre-write state overwrite the published file.
