...
排队执行后端写入时，如果 `KvUnit` 方法以 `StorageError('closed')` 拒绝，领域会进入终止状态，使缓存读取拒绝、排空写入链、关闭 unit，随后释放名称以便从介质重新打开。调用方的 `update` 转换函数抛出的错误不会触发该后端生命周期转换；其他被拒绝的写入不会改变领域内存，写入链仍可继续使用。
