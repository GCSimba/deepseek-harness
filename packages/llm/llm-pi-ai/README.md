...
- **Provider HTTP status is unavailable** — pi-ai error events do not expose a stable HTTP status across providers, so the adapter classifies stable harness error codes from flattened message text. An explicit `server_error` marker maps to `SERVER`; wording without a recognized marker or status remains `PI_AI_ERROR`.
