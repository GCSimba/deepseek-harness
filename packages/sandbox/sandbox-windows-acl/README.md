...
The native spawn boundary treats pipe pairs, process/thread/job handles, and temporary stdio inheritance bits as transferred ownership. If setup, launch, drain, or wait fails, it closes every handle it still owns, restores every inheritance bit it enabled, terminates any process returned in a malformed partial result, and preserves the primary Win32 error across best-effort cleanup.
